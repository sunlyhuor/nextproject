...
export default function CardComponent({ title , picture , link , button , discount , price , date }){
    ...
    return(
        <>
            <aside className="hover:scale-[1.08] relative hover:duration-300 max-w-sm bg-white min-[0px]:w-full sm:md:w-6/12 md:w-[48%] lg:w-3/12 xl:w-[24%] border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                    <Link className="w-[250px] h-[140px] block w-full overflow-hidden" href={link}>
                        <img className="rounded object-auto w-full h-[140px] hover:scale-110 hover:duration-300" src={picture} alt="Error" />
                    </Link>
                    <div className="p-5">
                        <Link href={link}>
                        ...
                        <div>
                            <span>
                                {price?price+"$":""} { (new Date() < date) ? sybol : "" } { (new Date() < date) ? price - ( discount * price /100 )+"$" : "" }
                            </span>
